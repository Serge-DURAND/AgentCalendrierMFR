...
def _orange(s: str) -> str:
    return f"{_ORG}{s}{_R}"

def _true_red(s: str) -> str:
    return f"{_B}{_RGE}{s}{_R}"
import logging
import signal
import atexit
import re
...
CLASS_ORDER = [
    ...
    "CAPA1JP A", "CAPA1JP B", "CAPA2 JP A", "CAPA2 JP B", "TJEP1", "TJEP2"
]
...
CYCLE_GROUPS = [
    ...
    ("CAPA",     ["CAPA1JP A", "CAPA1JP B", "CAPA2 JP A", "CAPA2 JP B"]),
    ("TJEP",     ["TJEP1", "TJEP2"]),
]
...
def is_dedouble(summary: str, description: str) -> bool:
    """
    Vrai dédoublement = classe scindée en 2 groupes, qui font le cours l'un
    après l'autre avec 2 formateurs différents (même intitulé de cours ou
    différent selon les cas). Le mot 'Dédoublement' dans la description
    suffit à le détecter.
    """
    return "doublement" in description.lower()
...
def _show_detail(classe: str, course_key: str):
    ...
    # Ancienne règle anti-faux-positif (retirée le 2026-08-28 : sur les cas
    # réels vérifiés, un dédoublement rare dans l'année reste un vrai
    # dédoublement — la fréquence annuelle n'a rien à voir avec sa validité).
    # Gardée en commentaire au cas où :
...
def load_vacances() -> dict:
    """Périodes de vacances par année scolaire : {"2026-2027": [["lundi", "vendredi"], ...]}."""
    try:
        with open(VACANCES_FILE, "r", encoding="utf-8") as f:
            return json.load(f)
    except FileNotFoundError:
        return {}
...
def run_menu_annual_analysis():
    ...
    # --- Détection des regroupements (même créneau, plusieurs classes) ---
    # Un cours "Regroupé" (ex: Term AP + Term CPH réunies) apparaît en double dans le
    # calendrier (une entrée par classe) pour la MÊME séance formateur : il ne faut la
    # compter qu'une fois (coefficient 0.5 par entrée dupliquée), sinon la charge du
    # formateur est comptée en double sur ces créneaux.
    slot_classes: Dict = {}
    for evts in _last_state.values():
        for ev in evts.values():
            ...
            if not parsed:
                continue
            classe, fil, mod, nom = parsed
            s_raw = ev.get("start_iso", "")
            e_raw = ev.get("end_iso", s_raw)
            slot_classes.setdefault((s_raw, e_raw, fil, mod, nom), set()).add(classe)
    regroupement_slots = {k for k, classes in slot_classes.items() if len(classes) > 1}

    # --- Collecte des heures formateur par jour (brutes, pondérées par regroupement) ---
    ...
    for evts in _last_state.values():
        for ev in evts.values():
            ...
            try:
                ...
                regroupe = (s_raw, e_raw, fil, mod, nom) in regroupement_slots
                dur_fmt  = dur_raw * (0.5 if regroupe else 1.0)
                d        = start.date()
                day_hours[d] = day_hours.get(d, 0.0) + dur_fmt
            except Exception:
                continue
...
# ---------------------------------------------------------------------
# Menu 9 : Répartition des cours sur l'année (tableau classe/cours x semaine)
# ---------------------------------------------------------------------
...
def run_menu_repartition():
    ...
    sy_start, sy_end = school_year_bounds(_selected_sy)
    first_monday = sy_start - datetime.timedelta(days=sy_start.weekday())
    nb_weeks = (sy_end - first_monday).days // 7 + 1
    today = datetime.datetime.now(TIMEZONE).date()

    objectifs = load_objectifs()
    _obj_cn = next(iter(_last_analysis), "")

    # Semaines de vacances (dates lundi-vendredi dans vacances.json, par année
    # scolaire — les vacances MFR tombent toujours sur des semaines entières).
    vac_ranges = load_vacances().get(_selected_sy, [])
    week_is_vac = []
    for i in range(nb_weeks):
        monday = first_monday + datetime.timedelta(weeks=i)
        is_vac = False
        for start_s, end_s in vac_ranges:
            try:
                v_start = datetime.date.fromisoformat(start_s)
                v_end   = datetime.date.fromisoformat(end_s)
            except (ValueError, TypeError):
                continue
            if v_start <= monday <= v_end:
                is_vac = True
                break
        week_is_vac.append(is_vac)

    # Position (caractère) de chaque semaine dans la ligne de données, et liste
    # des (position, mois) où démarre un nouveau mois — un espace de séparation
    # est inséré avant la 1re semaine de chaque mois (sauf la toute première).
    week_pos = []
    month_starts = []
    pos = 0
    prev_month = None
    for i in range(nb_weeks):
        monday = first_monday + datetime.timedelta(weeks=i)
        if i > 0 and monday.day <= 7:
            pos += 1
        if monday.month != prev_month:
            month_starts.append((pos, monday.month - 1))
            prev_month = monday.month
        week_pos.append(pos)
        pos += 1
    row_len = pos

    _MOIS_ABR3 = ["Jan", "Fév", "Mar", "Avr", "Mai", "Jun",
                  "Jul", "Aoû", "Sep", "Oct", "Nov", "Déc"]

    header_chars = [" "] * row_len
    for idx, (start_pos, month_idx) in enumerate(month_starts):
        next_pos = month_starts[idx + 1][0] if idx + 1 < len(month_starts) else row_len
        max_len = max(0, next_pos - start_pos - 1)   # 1 espace tampon avant le mois suivant
        label = _MOIS_ABR3[month_idx][:max_len]
    ...
    def _count_by_week(evts):
        """Applique la règle de comptage (Formateur ou Élèves) à une liste
        d'événements déjà filtrée, et retourne le tableau par semaine."""
        week_counts = [0] * nb_weeks
        if vue == "F":
            for start, end, dur, past, dedouble, regroupe in evts:
                idx = (start.date() - first_monday).days // 7
                if 0 <= idx < nb_weeks:
                    week_counts[idx] += 1
        else:
            week_ded = [False] * nb_weeks
            for start, end, dur, past, dedouble, regroupe in evts:
                idx = (start.date() - first_monday).days // 7
                if not (0 <= idx < nb_weeks):
                    continue
                if dedouble:
                    week_ded[idx] = True
                else:
    ...
    def _week_counts(classe, ck):
        """Compte les cours par semaine (grille). Vue Formateur : chaque
        créneau assuré compte. Vue Élèves : les séances dédoublées d'une même
        classe/matière/semaine ne comptent qu'une fois, même réparties sur
        plusieurs jours (un élève n'assiste qu'à un seul des groupes)."""
        evts = _merge_consecutive_events(_get_detail_events(classe, ck, today))
        evts = [e for e in evts if round(e[2] * 60) >= 30]
        return _count_by_week(evts)

    def _obj_blocks(classe, ck):
        """Nombre de cours (blocs de 2h) prévus pour cette classe/matière selon
        objectifs.json (heures 'fmt' en vue Formateur, 'eleve' en vue Élèves)."""
        obj = objectifs.get(_obj_cn, {}).get(_selected_sy, {}).get(classe, {}).get(ck, {})
        heures = float(obj.get("fmt" if vue == "F" else "eleve", 0))
        return heures / 2

    _REP_TOT_W = 12   # largeur colonne TOTAL ("Pos./Th." / ex. "12/10.5")

    def _print_table(rows):
        def _print_header():
            print()
            print()
            vue_lbl = "Vue Formateur" if vue == "F" else "Vue Élèves"
            print(f"  {_red(vue_lbl + ', affichage en Cours (blocs de 2h)')}")
            print(_REP_PREFIX + header_row + f"  {'Pos./Th.':>{_REP_TOT_W}}")
            print()
        ...
        for i, (classe, ck) in enumerate(rows, 1):
            ...
            fil, mod, nom = ck.split("|", 2)
            cl_lbl = classe if len(classe) <= _REP_CL_W else classe[:_REP_CL_W - 1] + "…"
            co_txt = course_label_short(mod, nom)
            co_lbl = co_txt if len(co_txt) <= _REP_CO_W else co_txt[:_REP_CO_W - 1] + "…"

            week_counts = _week_counts(classe, ck)
    ...
    while True:
        ...
        if cmd.upper() in ("VF", "VE"):
            vue = "F" if cmd.upper() == "VF" else "E"
            print(f"  {_red('Vue Formateur' if vue == 'F' else 'Vue Élèves')} activée.")
            continue
        if cmd.upper() == "T":
            rows = class_courses
        else:
            ...
            if len(parts) == 2 and parts[0].upper() == "C":
                needle = normalize_for_match(parts[1])
                rows = [(cl, ck) for cl, ck in class_courses if needle in normalize_for_match(cl)]
            elif len(parts) == 2 and parts[0].upper() == "M":
                needle = normalize_for_match(parts[1])
                rows = []
                for cl, ck in class_courses:
                    fil, mod, nom = ck.split("|", 2)
...
def print_main_menu(cal: str, sy: str):
    ...
    print("   ► 9 : Répartition des cours sur l'année")
    print()
    print("   ► Z : Changer d'année scolaire")
    print()
    print("   ► 0 : Quitter")
    print("-" * 64)
